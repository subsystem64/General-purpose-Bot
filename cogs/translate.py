...
class Translate(commands.Cog):
    ...
    @commands.command(pass_context=True, aliases=['translate','Translate','Tr'])
    async def tr(self, ctx, *, translate = None):
        ...
        usage = "Usage: `&tr [input] [from code (optional)] [to code]`".format(ctx.prefix)
        ...
        if result == trans:
            ...
            await Message.EmbedText(
                title="Error",
                description="The text returned from Google was the same as the text input.  Either the translation failed - or you were translating from/to the same language (en -> en)",
                color=ctx.author
            ).send(ctx)
            return
